...
The server uses [`intfloat/multilingual-e5-base`](https://huggingface.co/intfloat/multilingual-e5-base) for semantic search — a 278M-parameter multilingual model that produces 768-dimensional embeddings. It was chosen because it supports 100+ languages (important for a 13-language taxonomy), runs efficiently on CPU via ONNX quantisation (~10ms per query, ~350 MB memory), and belongs to the same E5 family already used by the parent Rijksmuseum MCP server for artwork embeddings, so the prefix conventions and quantisation pipeline carry over without changes. The base variant (768d) was preferred over the small variant (384d) for better discrimination between related concepts — relevant for a taxonomy where "crucifixion" and "deposition from the cross" need to be distinguished as separate results.

E5-base has a 512-token context window. The composite texts fed to the model (label + Dutch label + keywords + category path) top out at 478 tokens for the single most verbose notation in the entire taxonomy, with a mean of 122 and a 99th percentile of 298. The token budget is not a constraint.
